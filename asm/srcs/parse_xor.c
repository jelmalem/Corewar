...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/05/04 22:14:51 by hel-hadi          #+#    #+#             */
/*   Updated: 2017/05/06 17:23:44 by cfatrane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/asm.h"

static int		parse_xor_check(char **tab2, char *label)
{
...
int		parse_xor(char *str, char *label)
{
	...
	i = 0;	
	ft_printf("\033[31mXOR DETECT\033[0m\n");
	printf("[%s]\n", str);
	...
	// 	i++;
	// }
	if (ft_strcmp(tab[0], "xor") != 0 || tab[4] != NULL)
	{
	...
	if (parse_xor_check(tab, label) == 1)
	{
		printf("ERROR - SIGN\n");
		return (-1);
	}
